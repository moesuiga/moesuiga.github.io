...
想了解更多，可以查看 [TypeScript 官方文档手册](https://www.tslang.cn/docs/home.html)
愿你有一段愉快的 TypeScript 开发体验。

## 类型

> 注：这里所说的基础类型与复杂类型，是基于 JavaScript 的原始类型与引用类型来区分的，并非 TypeScript 文档中所说的基础类型
...
比如: `object`, `enum`枚举, `Array<T>`, `ArrayLike<T>`, Tuple元组, `Function` 等等，类型有很多，先不用太过关注这里列举出来的东西。
...
通过类型断言的方式来告诉 TypeScript 编译器：“我很清楚自己在干什么，相信我，它在这里就是这个类型”。
...
// 考虑到你可能会问，既然有两种类型断言的语法，那么另一种是什么？
// 这里就再写一个吧
let str: any = 'Hello TS';
// <string> 就是另一种类型断言，写在变量前面，告诉编译器，它就是字符串
...
举例来说，当你给一个变量初始声明并赋值一个数字时，那么 TypeScript 就可以直接推断认为它是一个 `number` 类型的数据，而不需要你再通过如上示例那样给变量标注 `: number` 来告诉 TypeScript 它是一个数字。
...
```ts
let dynamic: any = 123;
// 因为 dynamic 是 any，所以这里的 num 也是 any
let num = dynamic;
```

### 联合类型与交叉类型

就像上面介绍 `any` 类型时所说，有时一个变量可能不止一种类型，比如一个网络请求的返回数据，当服务能查到数据时，会返回一个数值类型的状态码，可是如果服务出错了，那就只能返回 `null` 了。
这种情况下，要怎么声明它既可能是一个数字，又可能是一个空值呢？
TypeScript 有提供一种联合类型: `A | B` 。
比如上面这种可能是数字，也可能是 `null` 的情况，就可以这样来声明：
...
在自定义一些复杂的类型时，接口是经常被使用到的内容，我认为可以说 TypeScript 中最常用的、最重要的类型就是接口。
...
当你以接口 `IPerson` 为类型声明一个对象时，它就必须包含上述三个属性，且值要符合其各自的类型。
...
TypeScript 支持两种索引签名：字符串和数字。【实际上，`Symbol` 也是可以的】
...
在 TypeScript 中，你可以强制一个类按照某个接口定义的描述来实现它，使用 `implements` 来实现一个接口。
...
可是如果我把它们定义为 `any` 类型，那样并不能表现出来返回类型与参数类型直接的关系，不论传入什么参数，函数的返回都是 `any` 类型。
可事实上，我们知道，如果传入 `string` ，它就会返回 `string`；如果传入 `number` ，它也会返回 `number`。
为此，我可以使用泛型来定义，先给函数声明一个泛型 A【A仅仅只是一个随便的命名，并非必须要命名为 A】，然后定义参数 `arg` 类型为 A。那么函数的返回类型自然也是 A。
...
从上面的示例中，我们来总结一下：
所谓 **可辨识联合** 就是多个具有相同单例属性名称但有不同字面量的类型，联合成一个类型，然后根据这个单例属性【即 *可辨识特征* 】的不同字面量来推断出它的具体类型【*类型保护*】。
